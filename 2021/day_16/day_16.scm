...
(define (parse-by-length lst)
  (receive (len lst) (apply values (parse-length lst 15))
...
(define (parse-by-count lst)
  (receive (len lst) (apply values (parse-length lst 11))
    (let loop ((lst lst) (cnt len) (acc '()))
      (if (= cnt 0)
        (list (flatten (reverse acc)) lst)
        (receive (value lst) (apply values (parse lst))
...
(define (parse-operator lst type-id)
  (receive (len-id . lst) (apply values lst)
    (receive (value lst)
      (apply values ((if (string=? len-id "0")
                       parse-by-length
                       parse-by-count)
                     lst))
...
(define (parse lst)
  (receive (version type-id lst) (apply values (parse-header lst))
