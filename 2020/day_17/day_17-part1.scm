...
(define (import-input path)
  (let ((lst (map string->list (read-lines (open-input-file path)))))
    (let ((w (length lst)) (h (length (list-ref lst 0))) (content (make-hash-table)))
      (do ((i 0 (+ i 1))) ((= i h))
        (do ((j 0 (+ j 1))) ((= j w))
          (let ((point (make-point i j 0)))
            (hash-table-set! content point (list-ref (list-ref lst i) j)))))
      (make-world content 0 h 0 w 0 1))))
...
(define (count-neighbors content i j k)
  (set! cnt 0)
  (do ((a -1 (+ a 1))) ((= a 2))
    (do ((b -1 (+ b 1))) ((= b 2))
      (do ((c -1 (+ c 1))) ((= c 2))
        (when (or (not (= a 0))
                  (not (= b 0))
                  (not (= c 0)))
          (let ((point (make-point (+ i a) (+ j b) (+ k c))))
...
(define (iterate-world input)
  (match input
    (($ world content h-start h-end w-start w-end d-start d-end)
       ...
       (do ((i (- h-start 1) (+ i 1))) ((= i (+ h-end 1)))
         (do ((j (- w-start 1) (+ j 1))) ((= j (+ w-end 1)))
           (do ((k (- d-start 1) (+ k 1))) ((= k (+ d-end 1)))
             (let ((cnt (count-neighbors content i j k)) (point (make-point i j k)))
               (case cnt
                 ((2))
...
(define (solve/1 input n)
  (let solve/1/h ((n n) (input input))
    (if (= n 0)
        (print (count-active input))
        (solve/1/h (- n 1) (iterate-world input)))))
