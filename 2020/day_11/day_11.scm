...
(define (iterate-world proc setting input H W)
  (let ((output (copy-world input H W)))
    (do ((i 0 (+ i 1))) ((= i H))
      (do ((j 0 (+ j 1))) ((= j W))
        (let ((cnt (proc input i j H W)))
          (case (vector-ref (vector-ref input i) j)
            ((#\#) (when (> cnt setting)
                     (vector-set! (vector-ref output i) j #\L)))
            ((#\L) (when (= cnt 0)
                     (vector-set! (vector-ref output i) j #\#)))))))
    output))

(define (solve proc setting input H W)
  (let solve/h ((input input))
    (let ((output (iterate-world proc setting input H W)))
      (if (equal? output input)
          (print (count-occupied input H W))
          (solve/h output)))))
